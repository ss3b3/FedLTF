...
from Core.Networks.ResNet import resnet18, resnet34, resnet50, resnet101, resnet152
...
def get_SSL_model(args):
    # return SimSiam(0.99, args)
    if args.dataset in ['mnist','fmnist']:
        input_channel = 1
    else:
        input_channel = 3
    if args.encoder_network == 'resnet18' :
        net = resnet18(num_classes = args.num_classes, input_channel = input_channel)
    elif args.encoder_network == 'resnet50':
        net = resnet50(num_classes = args.num_classes, input_channel = input_channel)
    else:
        raise NotImplementedError
    if args.SSL_method == 'simsiam':
        return SimSiamModel(net=net, stop_gradient=True)
    elif args.SSL_method == 'byol':
        return BYOLModel(net=net, stop_gradient=True, has_predictor=True,
                         predictor_network=TwoLayer)
    else:
        raise NotImplementedError
# def get_SSL_model(model, encoder_network,output_channel, predictor_network=TwoLayer):
#     mlp = False
#     T = 0.07
#     stop_gradient = True
#     has_predictor = True
...
#     elif model == SimSiam:
#         net = resnet50(num_classes = output_channel)
#         if encoder_network == RESNET50:
#             net = resnet50(num_classes = output_channel)
#         return SimSiamModel(net=net, stop_gradient=stop_gradient)
#     elif model == MoCo:
#         net = resnet18(num_classes = output_channel)
#         if encoder_network == RESNET50:
#             net = resnet50()
...
class SimSiam(nn.Module):
    def __init__(self, emam, args):
        ...
        self.backbone = SimSiam.get_backbone(args.model)
        self.backbone_k = SimSiam.get_backbone(args.model)
        self.dim_out, self.dim_in = self.backbone.fc.weight.shape
        dim_mlp = 2048
        self.backbone.fc = nn.Identity()
        self.backbone_k.fc = nn.Identity()
        dim_out, dim_in = self.dim_out, self.dim_in
        ...
        self.linear = Linear(nb_classes=args.num_classes, feat=dim_in)
